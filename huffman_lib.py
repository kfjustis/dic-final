...
'''
Source: http://code.activestate.com/recipes/576603-huffman-coding-
encoderdeconder/

Python 3 notes: used 2to3 to fix most issues, then used:
http://stackoverflow.com/questions/41913043/typeerror-not-supported-between-
instances-of-tuple-and-str to solve comparator issues with nodes.sort()
'''
...
class Decoder(object):
    ...
    def _decode(self):
        ...
        # Kynan: Convert decimal values back to chars!
        char_list = [chr(c) for c in string_buf]
        return ''.join(char_list) # return ''.join(string_buf)
    ...
    def decode_as(self, filename):
        ...
        # Kynan: byte-like object required for write()
        fout.write(decoded.encode())
        fout.close()
